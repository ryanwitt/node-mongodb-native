...
const makeCryptCommand = require('./cryptCommand').makeCryptCommand;
const isCryptEnabled = require('./cryptCommand').isCryptEnabled;
const cryptCommand = makeCryptCommand(_command);
...
function command(server, ns, cmd, options, callback) {
  ...
  if (isCryptEnabled(server)) {
    cryptCommand(server, ns, cmd, options, callback);
    return;
  }
